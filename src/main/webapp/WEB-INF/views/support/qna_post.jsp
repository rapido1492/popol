								...
								<article class="default">
									<div class="item-content">
										<header class="article-header clearfix">
											<h2 class="article-title" itemprop="headline">${ qna_post.qna_title }</h2>
										</header>
										...
										<aside class="article-aside clearfix">
											<dl class="article-info  muted">
												<dd class="createdby hasTooltip" itemprop="author" title=""
													...
													<i class="fa fa-user"></i> <span itemprop="name">작성자
														: ${qna_post.m_nick }</span> </span>
												</dd>
												<dd class="category-name hasTooltip" title=""
													data-original-title=" ${ qna_post.qna_title } ">
													<i class="fa fa-folder-open"></i> <span itemprop="genre">
														<c:if test="${qna_post.qna_category eq 1 }">
													공지사항
													</c:if> <c:if test="${qna_post.qna_category eq 2 }">
													긴급 공지사항
													</c:if>
													</span>
												</dd>
												<dd class="published hasTooltip" title=""
													...
													<time datetime="${qna_post.qna_date }"
														itemprop="datePublished">작성일 : ${qna_post.qna_date }
													</time>
												</dd>
											</dl>
										</aside>
										...
										<section class="article-intro clearfix" itemprop="articleBody">
											<div style="clear: both">${qna_post.qna_content }</div>
										</section>

										<section class="readmore text-right">
											<a class="btn btn-primary" href="qna_list.do?page=${page}"><span>목록</span></a>
										</section>

										<c:if test="${not empty qna_post.qna_answer }">
										답변 : <br>
										${qna_post.qna_answer}
										<br>
										<br>
										...
										<section class="textarea">
											<form action="qna_answer.do">
												<c:if test="${ empty qna_post.qna_answer }">
												답변 교체하기
												</c:if>
												<c:if test="${ not empty qna_post.qna_answer }">
												답변 쓰기
												</c:if>
												<textarea rows="100" id="qna_answer" name="qna_answer" cols="100"></textarea>
												<input type="hidden" id="qna_idx" name="qna_idx" value="${qna_post.qna_idx }">
